...
import {
	filterDismissedWorkspacePaths,
	filterWorkspacePaths,
	mergeWorkspacePaths,
	normalizeWorkspacePath,
	readDismissedWorkspacesFromWindow,
	readWorkspaceSelectionFromWindow,
	workspacePathsFromSessions,
	writeDismissedWorkspacesToWindow,
...
export default function Home() {
	...
	const activeHistorySessionId = (() => {
		...
		return thread?.historySession?.sessionId ?? thread?.activeSessionId ?? null;
	})();
	const activeThread =
...
function ChatThreadPane({
	...
	const [hasConnectedProvider, setHasConnectedProvider] = useState<
		boolean | null
	>(null);
	const [dismissedWorkspaces, setDismissedWorkspaces] = useState<string[]>(
		readDismissedWorkspacesFromWindow,
	);
	...
	// A fresh UI thread receives its durable Cline session id only after the
	// first send starts. Keep that association in the shell so deleting the row
	// from the sidebar can also retire the currently mounted chat pane.
	useEffect(() => {
		if (sessionId) {
	...
	useEffect(() => {
		setWorkspaces((current) => {
			const merged = filterDismissedWorkspacePaths(
				filterWorkspacePaths(mergeWorkspacePaths(knownWorkspacePaths, current)),
				dismissedWorkspaces,
			);
	...
	}, [dismissedWorkspaces, knownWorkspacePaths]);
	...
	const handleModelSettingsChange = useCallback(
		(settings: ModelRuntimeSettings) => {
			saveModelRuntimeSettings(settings);
			setConfig((prev) => ({
				...prev,
	...
	const listWorkspaces = useCallback(
		async (preferredWorkspace?: string): Promise<string[]> => {
			...
			return filterDismissedWorkspacePaths(
				filterWorkspacePaths(
	...
	const switchWorkspace = useCallback(
		async (workspacePath: string): Promise<boolean> => {
			...
			invalidateGitBranch();
			setDismissedWorkspaces((current) => {
				const next = current.filter(
					(path) =>
						normalizeWorkspacePath(path) !==
						normalizeWorkspacePath(resolvedWorkspace),
				);
				writeDismissedWorkspacesToWindow(next);
				return next;
			});
	...
	const removeWorkspace = useCallback(
		async (workspacePath: string): Promise<boolean> => {
			...
			const activeWorkspace = normalizeWorkspacePath(
				workspaceRef.current.workspaceRoot || workspaceRef.current.cwd || "",
			);
			...
			setDismissedWorkspaces((current) => {
				const next = mergeWorkspacePaths(current, [workspacePath]);
				writeDismissedWorkspacesToWindow(next);
				return next;
			});
			setWorkspaces((current) =>
				current.filter((path) => normalizeWorkspacePath(path) !== normalized),
			);
			return true;
		},
			...
			setConfig((prev) =>
				prev.model === nextModel
					...
					: {
							...
							modelSettings: readModelRuntimeSettings(
								prev.provider,
								nextModel,
							) ?? { provider: prev.provider, model: nextModel },
						},
			),
		[setConfig],
	);
	...
	const handleAutoApproveChange = useCallback(
		(value: AutoApprove) => {
			try {
				...
				setConfig((prev) => ({ ...prev, autoApprove: value }));
			} catch {
				toast({ title: "保存自动允许设置失败", variant: "destructive" });
			}
		},
		[setConfig],
	);
		...
		(nextProvider: string) =>
			setConfig((prev) => {
				...
				return {
					...
					thinking: null,
					reasoningEffort: undefined,
					modelSettings: readModelRuntimeSettings(nextProvider, prev.model) ?? {
						provider: nextProvider,
						model: prev.model,
					},
					apiKey: nextApiKey,
	...
	useEffect(
		() =>
			desktopClient.subscribe("session_title_updated", (payload) => {
				if (!payload || typeof payload !== "object") return;
				const record = payload as { sessionId?: unknown; title?: unknown };
				if (
					record.sessionId === activeSessionForTitle &&
	...
	return (
		<WorkspaceProvider value={workspaceContextValue}>
			...
			<AttachmentDropZone
				className={
					isWelcomeState
						? "grid h-full min-h-0 min-w-0 flex-1 grid-cols-[minmax(0,1fr)] grid-rows-[minmax(0,1fr)] overflow-hidden"
						: "grid h-full min-h-0 min-w-0 flex-1 grid-cols-[minmax(0,1fr)] grid-rows-[minmax(0,1fr)_auto] overflow-hidden"
				}
				onAttachFiles={handleAttachFiles}
			>
				{!isWelcomeState ? (

...
export class DefaultRuntimeBuilder implements RuntimeBuilder {
	...
	async build(input: RuntimeBuilderInput): Promise<RuntimeEnvironment> {
		...
		// Clone before expanding category policies; never mutate caller settings.
		...
		if (normalized.enableTools) {
			...
			if (
				...
				// Opt-in category policy applies only to tools actually loaded from
				// MCP servers, not names guessed from prefixes or other plugins.
				const mcpPolicy = effectiveToolPolicies["mcp:*"];
				if (mcpPolicy) {
					for (const tool of mcpRuntime.tools) {
						effectiveToolPolicies[tool.name] = {
							...mcpPolicy,
							...effectiveToolPolicies[tool.name],
						};
					}

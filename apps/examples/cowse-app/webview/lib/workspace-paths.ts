...
export const DISMISSED_WORKSPACES_STORAGE_KEY =
	"cline.code.dismissed-workspaces.v1";
...
export function filterDismissedWorkspacePaths(
	paths: readonly string[],
	dismissedPaths: readonly string[],
): string[] {
	const dismissed = new Set(dismissedPaths.map(normalizeWorkspacePath));
	return paths.filter((path) => !dismissed.has(normalizeWorkspacePath(path)));
}

export function readDismissedWorkspacesFromWindow(): string[] {
	if (typeof window === "undefined") {
		return [];
	}
	try {
		const parsed = JSON.parse(
			window.localStorage.getItem(DISMISSED_WORKSPACES_STORAGE_KEY) || "[]",
		);
...
export function writeDismissedWorkspacesToWindow(
	paths: readonly string[],
): void {
	if (typeof window === "undefined") {
		return;
	...
	try {
		window.localStorage.setItem(
			DISMISSED_WORKSPACES_STORAGE_KEY,
			JSON.stringify(filterWorkspacePaths(mergeWorkspacePaths(paths))),
		);
	} catch {
		// Keep workspace selection functional when storage is unavailable.
	}
}

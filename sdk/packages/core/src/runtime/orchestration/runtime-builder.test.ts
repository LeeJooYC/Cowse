...
Use the review guidance.`,
	...
	it.each([
		true,
		false,
	])("applies MCP category approval only to discovered MCP tools: %s", async (autoApprove) => {
  ...
  while (true) {
    ...
    handle(message);
		...
		try {
			...
			const runtime = await new DefaultRuntimeBuilder().build({
				config: makeBaseConfig({ toolPolicies: policies }),
			});
			expect(runtime.tools.map((tool) => tool.name)).toContain("mock__echo");
			expect(runtime.toolPolicies?.mock__echo).toEqual({ autoApprove });
			expect(runtime.toolPolicies?.["*"]).toEqual({ autoApprove: false });
			expect(policies).not.toHaveProperty("mock__echo");
			await runtime.shutdown("test");
			const explicit = await new DefaultRuntimeBuilder().build({
				config: makeBaseConfig({
					toolPolicies: {
						...policies,
						mock__echo: { autoApprove: !autoApprove },
					},
				}),
			});
			expect(explicit.toolPolicies?.mock__echo).toEqual({
				autoApprove: !autoApprove,
			});
			await explicit.shutdown("test");
		} finally {

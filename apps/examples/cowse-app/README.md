...
本目录是牛马的桌面应用源码。当前仓库的安装、构建和打包入口以根目录 [README](../../../README.md) 为准。下方保留的上游技术说明仅供参考，其中 Cline 官方发布流程不适用于本仓库。

Tauri desktop shell + Bun sidecar backend + Next.js UI for running and inspecting Cline chat sessions.

## 本地模型上下文和思考设置

输入框右下角的上下文入口始终可用。弹窗先查询所选模型上限，服务端未返回或查询失败时提供 16K、32K、64K、128K、256K 五档可选预算；此范围不作为模型真实上限保存。有上限时按倍增分档，最后一档为该上限（K = 1024 Token）。无保存值时默认 128K，上限不足则下调。拖动或点击档位直接保存，下次发送生效，运行中不可更改。预算按供应商和模型保存，投影到 Cline 的 `providerConfig.modelInfo.contextWindow/maxInputTokens`；Core 仍负责上下文管理和压缩。预算不是输出长度，也不能扩大服务端的实际容量。

上下文弹窗不再提供模型目录、读取配置、思考档位勾选和保存设置入口。思考菜单仅使用 Cline 提供的 reasoningOptions，不再解析模型文件或使用牛马保存的自定义能力声明；旧设置中的这类字段在读取、保存和传给 Core 时忽略。上下文预算保持不变，修改预算不会重置已选思考强度。

思考“默认”不产生请求级覆盖；恢复默认使用 Core 的 null 清除语义，避免 JSON 丢失 undefined 后遗留旧档位。切换模型或重新打开会话时回到默认，避免上个模型的 effort 泄漏。参数编码由 Cline 自身的模型调用层处理，不再保留牛马添加的本地模型关闭协议规则。

## Dev Commands

From `apps/examples/cowse-app/`:

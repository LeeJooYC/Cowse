...
function actionLabelFor(
	...
	if (!ready) return "正在检查…";
	if (state?.status === "installing") return "正在安装…";
	if (state?.status === "uninstalling") return "正在卸载…";
		...
		<ScrollArea className="h-full min-w-0 flex-1">
			<div className="grid max-w-3xl gap-6 px-8 py-8 max-[900px]:px-5">
				...
				{requiredEnv.length > 0 || optionalEnv.length > 0 ? (
					<section className="grid gap-2">
						<h2 className="text-sm font-semibold text-foreground">环境配置</h2>
	...
	return (
		<div className="page-content flex h-full min-h-0 flex-col">
			<PageHeader
				className="shrink-0"
				title="扩展市场"
				description="浏览并安装技能、MCP 服务和插件，为牛马添加更多能力。已安装的内容可在左侧“已安装”中管理。"
			/>
...
function MarketplaceDirectoryView() {
						...
						<div className="flex flex-wrap gap-1">
							...
							{hiddenTagCount > 0 || tagsExpanded ? (
								<button
									...
									type="button"
								>
									{tagsExpanded ? "收起" : `还有 ${hiddenTagCount} 项`}
								</button>
							) : null}
						</div>
					) : null}

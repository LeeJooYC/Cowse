	...
	sqliteIt(
		...
		async () => {
			...
			const workspaceDir = mkdtempSync(
				join(tmpdir(), "delete-root-session-workspace-"),
			);
			const workspaceFile = join(workspaceDir, "keep-me.txt");
			writeFileSync(
				workspaceFile,
				"project data must survive session deletion",
			);
			tempDirs.push(dbDir, sessionsDir, workspaceDir);
			...
			store.run(`UPDATE sessions SET messages_path = ? WHERE session_id = ?`, [
				workspaceFile,
				sessionId,
			]);

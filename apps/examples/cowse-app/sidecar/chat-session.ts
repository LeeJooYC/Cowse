...
function createLiveSession(
	...
	return {
		...
		compactionConfigured: overrides?.compactionConfigured ?? true,
...
/** Core owns compaction; the desktop host only opts in and reads Core's preferences. */
export function resolveDesktopCompactionConfig(
	settings: Pick<
		ReturnType<typeof readGlobalSettings>,
		"compactionEnabled" | "compactionStrategy"
	> = readGlobalSettings(),
) {
	return {
		enabled: settings.compactionEnabled !== false,
		strategy: settings.compactionStrategy ?? "agentic",
	};
}
...
export function buildCoreSessionConfig(config: JsonRecord): JsonRecord {
	...
	const reasoningEffort =
		thinking === false || config.thinking === null
			? undefined
	...
	return {
		...
		providerConfig: config.modelSettings
			? modelSettingsProviderConfig(config)
...
export function buildSessionConnectionUpdate(
	...
	if (config.modelSettings)
		update.providerConfig = modelSettingsProviderConfig(config);
	// null is intentional: undefined is stripped by JSON and means "keep" in Core.
...
export function mergeSessionConfig(
	...
	return {
		...currentConfig,
		...updates,
		...(updates.thinking === null || updates.thinking === false
...
async function handleStart(
	...
	// The webview allocates an ID before starting even a brand-new chat.
	// Check persistence BEFORE start() creates the record; an ID alone is not
	// evidence that this is a resumed session.
	const existingSession = requestedSessionId
		? await manager.get(requestedSessionId)
		: undefined;
	...
	session.autoTitleEligible = !existingSession && !initialMessages;
	ctx.logger?.log("Desktop session automatic title eligibility", {
		sessionId,
		eligible: session.autoTitleEligible,
	});
...
async function handleAttach(
	...
	// Locally driven sessions already receive Core events. Re-entering their
	// view must not enable the parallel raw Hub projection (duplicate deltas),
	// or replace the LiveSession object still held by an in-flight send/queue.
	const needsHubAttachment = !existing || existing.attachedViaHub;
	if (ctx.hubClient && needsHubAttachment) {
	...
	if (needsHubAttachment)
		ctx.liveSessions.set(
			...
			createLiveSession(attachedConfig, {
				...
				attachedViaHub: true,
				// An existing Hub runtime may predate desktop auto-compaction.
				// Migrate on its next idle send, never by interrupting a running turn.
				compactionConfigured: existing?.compactionConfigured ?? false,
	...
	return {
		...
		model: session.model,
		mode: resolveDesktopSessionMode(attachedConfig),
		...(attachedConfig.autoApprove !== undefined ||
		typeof attachedConfig.autoApproveTools === "boolean"
			? {
					autoApprove: resolveAutoApprove(
						attachedConfig.autoApprove,
						attachedConfig.autoApproveTools !== false,
					),
				}
			: {}),
		cwd: session.cwd,
...
async function handleSend(
	...
	const needsCompactionSetup = Boolean(
		session?.compactionConfigured === false &&
			!session.busy &&
			!ACTIVE_WORKSPACE_SESSION_STATUSES.has(session.status) &&
			session.promptsInQueue.length === 0 &&
			delivery !== "queue" &&
			delivery !== "steer",
	);
	const nextConfig = request.config
		? mergeSessionConfig(session?.config ?? {}, request.config)
		: needsCompactionSetup
			? session?.config
			: undefined;
	// Core connection updates do not change the tool preset or system prompt.
	// Reuse the existing history-preserving rebuild for mode/language changes.
		...
		Boolean(
			session &&
				...
				(hasProviderChanged(session.config, request.config) ||
					// An attached older session may not expose its runtime mode. Rebuild
					// on its first explicit mode selection instead of assuming Act.
					(session.attachedViaHub &&
						session.config.mode === undefined &&
						request.config.mode !== undefined) ||
					resolveDesktopSessionMode(session.config) !==
						resolveDesktopSessionMode(nextConfig ?? {}) ||
					normalizePreferredLanguage(session.config.preferredLanguage) !==
						normalizePreferredLanguage(nextConfig?.preferredLanguage) ||
					!isDeepStrictEqual(
						resolveToolPolicies(session.config),
						resolveToolPolicies(nextConfig ?? {}),
					)),
		);
	...
	if (session) {
		if (ownsBusyState) {
			if (session.autoTitleEligible && !session.autoTitlePrompt)
				session.autoTitlePrompt = prompt.slice(0, 1800);
	...
	try {
		...
		if (session && ownsBusyState) {
			session.status = "idle";
			if (result) setCompactionStatus(ctx, sessionId, false);
		...
		setCompactionStatus(ctx, sessionId, false);
		if (session && ownsBusyState) {
...
> = {
	start: handleStart,
	attach: handleAttach,
	compaction_status: async (ctx, request) => ({
		sessionId: request.sessionId,
		active: Boolean(
			ctx.liveSessions.get(request.sessionId ?? "")?.isCompacting,
		),
	}),
	send: handleSend,
	stop: handleStop,

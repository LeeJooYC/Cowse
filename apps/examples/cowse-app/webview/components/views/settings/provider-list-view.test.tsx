...
describe("ProviderDetailContent models", () => {
	...
	it("requests a fresh model list when Refresh models is clicked", async () => {
		...
		await act(async () => {
			container
				.querySelector<HTMLButtonElement>('[aria-label="刷新模型"]')
				?.click();
		});
	...
	it("always shows the back control, including the panel variant", async () => {
		const onBack = vi.fn();
		await act(async () => {
			root.render(
				<ProviderDetailContent
		...
		const backButton = container.querySelector<HTMLButtonElement>(
			'button[aria-label="返回供应商列表"]',
		);
		expect(backButton).not.toBeNull();
		expect(backButton?.classList).not.toContain(
			"provider-detail-responsive-back",
		);
	...
	it("persists favorites, sorts them first, and adds models", async () => {
		...
		await act(async () => {
			container
				.querySelector<HTMLButtonElement>('[aria-label="收藏 Beta"]')
				?.click();
		});
		expect(
			Array.from(
		...
		expect(
			container.querySelector('[aria-label="取消收藏 Beta"] svg')?.classList,
		...
		await act(async () => {
			container
				.querySelector<HTMLButtonElement>('[aria-label="新模型 ID"] + button')
				?.click();
		});
...
describe("ProviderListContent", () => {
	...
	it("groups providers into Configured, Popular, and All providers", async () => {
		...
		expect(headings).toEqual(["已配置", "常用", "全部供应商"]);
		// No per-provider enable toggles anymore.
		expect(container.querySelector('[role="switch"]')).toBeNull();
		expect(container.textContent).toContain("已配置 1 个");
...
describe("ProviderDetailContent auth flows", () => {
	...
	it("offers status recovery while waiting for browser login", async () => {
		const refresh = vi.fn();
		await act(async () => {
			root.render(
				<ProviderDetailContent
		...
		expect(container.textContent).toContain("下方模型列表不代表已登录");
		await act(async () => {
			[...container.querySelectorAll("button")]
				.find((b) => b.textContent === "刷新登录状态")
				?.click();
		});
	...
	it("shows audio icons beside transcription-capable models", async () => {
		...
		expect(
			container.querySelectorAll(
				'[role="img"][aria-label="支持音频"] .lucide-mic',
			),
		).toHaveLength(1);
	});

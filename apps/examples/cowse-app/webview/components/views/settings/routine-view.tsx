...
export function RoutineSchedulesContent({
	...
	const openCreateDialog = async (template?: RoutineTemplate) => {
		...
		setCreateForm({
			...
			prompt:
				template?.prompt ?? "审查昨天提交的 PR 并总结问题。",
	...
	return (
		<PageFrame>
			<PageHeader
				description="按计划运行智能体，执行每日摘要、代码审查等周期性自动化任务。"
				title="计划任务"
				actions={
					<>
						<Button
			...
			{isLoading ? (
				<PageEmptyState>正在加载计划任务…</PageEmptyState>
			) : sortedSchedules.length === 0 ? (
				<PageEmptyState>
					还没有计划任务。可以从下方建议开始，也可以新建自己的计划。
				</PageEmptyState>
			) : (
						...
						return (
							...
							>
								...
								<div className="mt-2.5 ml-5.5 flex flex-col gap-1 text-xs text-muted-foreground">
									...
									<p>
										<span className="text-muted-foreground/70">上次结果：</span>{" "}
			...
			>
				...
				>
					<DialogHeader>
						<DialogTitle>{viewingSchedule?.name ?? "计划任务"}</DialogTitle>
					</DialogHeader>
					{viewingSchedule && (
						<div className="flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto">
							...
							{!showAllViewingRuns && viewingExecutions.length > 3 ? (
								<Button
									...
									variant="ghost"
								>
									显示全部 {viewingExecutions.length} 条记录
			...
			>
				<AlertDialogContent>
					<AlertDialogHeader>
						<AlertDialogTitle>删除计划任务</AlertDialogTitle>
						<AlertDialogDescription>
							此操作将删除“{schedulePendingDelete?.name ?? "该计划任务"}
							”，并取消后续计划运行。
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
			...
			>
				<DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
					<DialogHeader>
						...
						<DialogDescription>
							{editingSchedule ? "更新此计划任务。" : "创建一个计划任务。"}
						</DialogDescription>
					</DialogHeader>
					...
					<div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
						...
						<div className="space-y-2">
							<Label htmlFor="routine-tags">标签（用逗号分隔，可选）</Label>
							<Input
								id="routine-tags"

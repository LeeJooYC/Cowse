...
function handleAgentEvent(
	...
	switch (event.type) {
		...
		case "notice": {
			const compacting = compactionNoticeState(event);
			if (compacting !== undefined)
				setCompactionStatus(ctx, sessionId, compacting);
			emitChunk(
				ctx,
				sessionId,
				"chat_core_log",
		...
		case "done": {
			...
			if (event.reason === "completed")
				void generateSessionTitle(ctx, sessionId, event.text);
			const session = ctx.liveSessions.get(sessionId);
			if (session) {
				session.busy = false;
...
export function handleCoreSessionEvent(
	...
	switch (event.type) {
		...
		case "ended": {
			...
			if (reason === "completed") void generateSessionTitle(ctx, sessionId);
			break;
		}
		case "hook": {
		...
		case "status": {
			...
			if (status !== "running") setCompactionStatus(ctx, sessionId, false);
			const session = ctx.liveSessions.get(sessionId);
			if (session) {
...
export function handleHubLiveEvent(
	...
	switch (event.event) {
		case "session.notice": {
			const agent = event.payload?.agent as
				| { parentAgentId?: unknown }
				| undefined;
			const compacting = compactionNoticeState({
				...event.payload,

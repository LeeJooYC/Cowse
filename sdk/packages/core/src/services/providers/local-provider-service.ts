...
function modelSourceRequestHeaders(
...
export async function addLocalProvider(
	...
	const modelIds = await resolveModelIds({
		...
		modelSourceHeaders: modelSourceRequestHeaders(apiKey, normalizedHeaders),
...
export async function updateLocalProvider(
	...
	const modelIds = await resolveModelIds({
		...
		modelSourceHeaders: modelSourceRequestHeaders(
			effectiveApiKey,
			effectiveHeaders,
		),
...
export async function refreshProviderModelsFromSource(
	...
	// The persisted registry is now fresh. Do not merge it with the previous
	// endpoint snapshot (or let an older in-flight request repopulate it).
	clearPublicModelsCatalogCache({ providerId: id, baseUrl });

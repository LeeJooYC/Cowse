...
- pre-request compaction and inference resolve the selected model through the same provider-config overlay; session `modelInfo` and input-budget overrides take precedence over catalog limits, including after resume and connection updates
...
# MCP approval category

Hosts may set `toolPolicies["mcp:*"]` as an opt-in policy for configured MCP tools (including Agent Plugin MCP servers). The runtime builder expands it only for tools returned by MCP discovery. Explicit per-tool policies take precedence; unrelated tools keep their existing policies. Tool approval remains in the standard agent/Hub approval flow.

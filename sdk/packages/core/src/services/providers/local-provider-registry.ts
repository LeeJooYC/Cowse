...
import { resolveModelsSourceUrl } from "./model-source";
...
export function registerCustomProvider(
	...
	const existingProvider =
		LlmsModels.getProviderCollectionSync(providerId)?.provider;
	// Older models.json entries may predate a built-in provider's live model
	// source. Preserve that Cline metadata while rebasing it to the saved URL,
	// otherwise registering the stored overlay makes Refresh silently inert.
	const modelsSourceUrl =
		entry.provider.modelsSourceUrl ??
		resolveModelsSourceUrl(
			entry.provider.baseUrl,
			existingProvider?.baseUrl,
			existingProvider?.modelsSourceUrl,
		);

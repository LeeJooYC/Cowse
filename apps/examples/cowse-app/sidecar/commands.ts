...
export async function handleCommand(
	...
	if (command === "prepare_full_quit") {
		if (!options?.connection?.data?.canApproveTools) {
			throw new Error("full quit requires a trusted desktop connection");
		}
	...
	if (command === "update_chat_session_title") {
		...
		if (liveSession) liveSession.autoTitleCancelled = true;
		await withTitleWrite(sessionId, async () => {
			const backend = await resolveSessionBackend({ backendMode: "local" });
	...
	if (command === "delete_chat_session" || command === "delete_cli_session") {
		...
		for (const path of [
			...
			if (typeof path === "string" && path.trim().length > 0) {
				...
				if (isPathInsideDirectory(artifactDirectory, sessionDataRoot)) {
					directoryCandidates.add(artifactDirectory);
				}
			}
		}
		...
		for (const path of [
			...
			(v): v is string =>
				typeof v === "string" &&
				v.length > 0 &&
				isPathInsideDirectory(v, sessionDataRoot),
		)) {
	...
	// ── Provider management ────────────────────────────────────────────
	if (command === "get_model_context_limit") {
		...
		if (provider === "openai-compatible") {
			return {
				limit: config
					? await queryCompatibleContextLimit(model, config)
					: undefined,
			};
		}
		const result = await getLocalProviderModels(provider, config);
		return {
			limit: result.models.find((entry) => entry.id === model)?.contextWindow,
		};
	}
	if (command === "list_provider_catalog") {
		...
		return {
			...
			providers: catalog.providers.map((provider) => {
				const knownModels = manager.getProviderConfig(provider.id)?.knownModels;
				return {
					...provider,
	...
	if (command === "list_provider_models") {
		...
		const knownModels = manager.getProviderConfig(providerId)?.knownModels;
		return {
			...result,
	...
	if (command === "run_provider_oauth_login") {
		...
		// Notify other mounted views only after Core has persisted credentials.
		// Never broadcast tokens or browser callback parameters.
		broadcastEvent(ctx, "provider_auth_changed", { provider: providerId });
		return result;
	}
	...
	// ── Local proxy (Surge / Clash) ────────────────────────────────────
	if (command === "get_proxy_config") {
		return readProxyConfig();
	}
	if (command === "detect_local_proxy") {
		return await detectLocalProxies();
	}
	if (command === "save_proxy_config") {
		const config = args?.config as Partial<ProxyConfig> | undefined;
		if (!config || typeof config.mode !== "string") {
			throw new Error("代理配置无效");
		}
		const mode = config.mode as ProxyConfig["mode"];
		if (mode !== "off" && mode !== "auto" && mode !== "manual") {
			throw new Error("代理模式无效");
		}
		const protocol = (config.protocol ?? "http") as ProxyProtocol;
		if (protocol !== "http" && protocol !== "https" && protocol !== "socks5") {
			throw new Error("代理协议无效");
		}
		if (protocol === "socks5") {
			throw new Error(
				"当前版本暂不支持 SOCKS5 代理，请使用 HTTP 代理端口（如 Clash 的混合端口 7892 或 Surge 的 HTTP 端口 6152）。",
			);
		}
		...
		writeProxyConfig(normalized);
		applyProxyConfig(normalized);
		return normalized;
	}

...
async function getPublicProviderModels(
	...
	const request = fetchModelIdsFromSource(sourceUrl, providerId)
		.then((modelIds) => {
			...
			// A refresh may have invalidated this request while it was running.
			// Its callers can finish, but it must not restore the stale cache.
			if (PUBLIC_MODELS_IN_FLIGHT.get(cacheKey) === request) {
...
export function clearPublicModelsCatalogCache(target?: {
	providerId: string;
	baseUrl?: string;
}): void {
	if (target) {
		const key = resolvePublicCacheKey(target.providerId, target);
		PUBLIC_MODELS_CACHE.delete(key);
		PUBLIC_MODELS_IN_FLIGHT.delete(key);
		return;
	}

...
beforeEach(async () => {
	...
	// Session-start mocks below return only an ID; supply an explicit workspace
	// instead of relying on the removed process-cwd default for Just chat.
	await act(async () =>
		current.setConfig((prev) => ({
			...prev,
...
describe("useChatSession", () => {
	it("keeps the prompt above live output arriving while a running session attaches", async () => {
		...
		invokeMock.mockImplementation(
			async (command: string, args?: Record<string, unknown>) => {
				if (command === "chat_session_command") {
					...
					if (request?.action === "attach") return attaching.promise;
		...
		try {
			...
			await act(async () => {
				attaching.resolve({ status: "running" });
				await hydration;
			});
			expect(
			...
			expect(current.messages[1].content).toBe("你好，我是牛马");
		} finally {
			vi.useRealTimers();
		}
	});
	...
	it("starts a fresh chat without inheriting a remembered or process workspace", async () => {
		await act(async () => root.unmount());
		window.localStorage.setItem(

...
import { readPreferredLanguage } from "@/lib/preferred-language";
...
export function normalizeRuntimeConfig(
	...
	const clearReasoning = config.thinking === null || config.thinking === false;
	const thinking = clearReasoning
		? config.thinking
	...
	return {
		...config,
		// Read at send time so existing sessions pick up General preferences.
		preferredLanguage: readPreferredLanguage(),
		workspaceRoot: normalizedWorkspaceRoot,
		cwd: normalizedCwd || normalizedWorkspaceRoot,
		thinking,
		reasoningEffort: clearReasoning ? undefined : config.reasoningEffort,
	};
}

...
describe("useSessionHistory session mapping", () => {
	it("updates the sidebar immediately when an automatic title arrives", async () => {
		await act(async () => root.render(<HookHarness />));
		await flush();
		await act(async () => {
			pendingLists[0].resolve([
				{ ...sessionRow("s1"), metadata: { title: "很长的原始提问" } },
			]);
			await Promise.resolve();
		});
		const subscription = (
			subscribeMock.mock.calls as unknown as Array<
				[string, (value: unknown) => void]
			>
		).find(([name]) => name === "session_title_updated")!;
		await act(async () =>
			subscription[1]({ sessionId: "s1", title: "添加本地代理" }),
		);
		expect(current.threads.find((t) => t.id === "s1")?.title).toBe(
			"添加本地代理",
		);
		expect(
			current.sessions.find((s) => s.sessionId === "s1")?.metadata?.title,
		).toBe("添加本地代理");
	});

...
import { readModelRuntimeSettings } from "@/lib/model-runtime-settings";
import {
...
import { registerHostHomeDirectory } from "@/lib/workspace-paths";
	...
	const reset = useCallback(async () => {
		...
		setConfig((prev) => {
			...
			return {
				...
				model: initial.model,
				modelSettings: initial.modelSettings,
		...
		async (session: SessionHistoryItem) => {
			...
			setConfig((prev) => ({
				...
				modelSettings: readModelRuntimeSettings(
					session.provider || prev.provider,
					session.model || prev.model,
				) ?? {
					provider: session.provider || prev.provider,
					model: session.model || prev.model,
				},
			...
			try {
				...
				const attached = await desktopClient
					.invoke<{
						...
						mode?: "act" | "plan";
						autoApprove?: ChatSessionConfig["autoApprove"];
						prompt?: string;
					}>("chat_session_command", {
						request: {
							action: "attach",
				...
				setConfig((prev) => ({
					...
					mode: attached?.mode === "plan" ? "plan" : "act",
					autoApprove: attached?.autoApprove ?? prev.autoApprove,
					model: attached?.model || session.model || prev.model,
					modelSettings: readModelRuntimeSettings(
						attached?.provider || session.provider || prev.provider,
						attached?.model || session.model || prev.model,
					) ?? {
						provider: attached?.provider || session.provider || prev.provider,
						model: attached?.model || session.model || prev.model,
					},
				...
				if (session.prompt?.trim()) {
					synthesized.push({
						...
						// The prompt predates any deltas received while history/attach
						// were loading; stamping it now sorts it below the answer.
						createdAt: hydrationStartedAt - 1,
					});
				}
				void refreshPromptsInQueue(session.sessionId);

...
export class UnifiedSessionPersistenceService {
	...
	async deleteSession(sessionId: string): Promise<{ deleted: boolean }> {
		...
		if (!row.isSubagent) {
			...
			await Promise.allSettled(
				children.map(async (child) => {
					...
					this.unlinkManagedArtifactIfExists(child.messagesPath);
		...
		this.unlinkManagedArtifactIfExists(row.messagesPath);
		...
		if (row.isSubagent) {
			...
			for (const path of [row.messagesPath]) {
				if (
					typeof path === "string" &&
					path.trim().length > 0 &&
					this.isManagedArtifactPath(path)
				) {
	...
	private isManagedArtifactPath(path: string): boolean {
		const relation = relative(
			resolve(this.manifestStore.ensureSessionsDir()),
			resolve(path),
		);
		return (
	...
	private unlinkManagedArtifactIfExists(path: string | null | undefined): void {
		if (path && this.isManagedArtifactPath(path)) {
			unlinkIfExists(path);
		}
	}

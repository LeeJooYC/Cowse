...
describe("SessionRuntime message preparation", () => {
	...
	it.each([
		false,
		true,
	])("compacts resumed history against the configured budget (connection update: %s)", async (updateConnection) => {
		const compact = vi.fn(async () => ({
			messages: [{ role: "user" as const, content: "Retained task summary" }],
		}));
		...
		const session = new SessionRuntime(
			makeAgentConfig({
				...
				providerConfig: updateConnection
					? undefined
					: {
							providerId: "anthropic",
							modelId: "claude-3-5-sonnet",
		...
		if (updateConnection) {
			await session.run("Initial turn before changing the budget");
		...
		await session.continue("One more question");
		const result = await configs.at(-1)?.prepareTurn?.({
			agentId: "agent-1",
			iteration: 1,
			messages: [
				{
				...
				// Several ordinary turns: a single oversized assistant message is
				// intentionally truncated by MessageBuilder before compaction.
				...Array.from({ length: 80 }, (_, index) => [
					...
					},
					{
						...
						content: [{ type: "text" as const, text: "x".repeat(10_050) }],
						createdAt: 3 + index * 2,
					},
				]).flat(),
				{
		...
		expect(compact).toHaveBeenCalledWith(
			expect.objectContaining({
				budget: expect.objectContaining({
					request: expect.objectContaining({ maxInputTokens: 262_144 }),
				}),
			}),
		);
		expect(result?.messages).toHaveLength(1);

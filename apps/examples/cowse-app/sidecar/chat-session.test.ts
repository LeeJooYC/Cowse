...
import {
	...
	resolveDesktopCompactionConfig,
	resolveToolPolicies,
	resolveSystemPrompt,
...
import { handleCoreSessionEvent, handleHubLiveEvent } from "./context";
import type { SidecarContext } from "./types";
import * as sessionTitle from "./session-title";

describe("desktop Core compaction configuration", () => {
	it("defaults to Core's agentic strategy and respects saved choices", () => {
		expect(resolveDesktopCompactionConfig({})).toEqual({
			enabled: true,
			strategy: "agentic",
		});
		expect(
		...
		).toEqual({ enabled: false, strategy: "basic" });
		expect(
			resolveDesktopCompactionConfig({ compactionStrategy: "basic" }),
		).toEqual({ enabled: true, strategy: "basic" });
	});
	it("passes the resolved settings into every Core session config", () => {
		expect(
			buildCoreSessionConfig({ provider: "openai-codex", model: "test" })
				.compaction,
		).toEqual(resolveDesktopCompactionConfig());
	});
});

describe("resolveDesktopSessionMode", () => {
	it("rejects malformed approval settings instead of granting permissions", () => {
		expect(() =>
			resolveToolPolicies({ autoApprove: { read: true } }),
		).toThrow();
	});
	it("maps approval categories without enabling unknown tools automatically", () => {
		const autoApprove = {
			read: true,
			edit: false,
			commands: false,
		...
		const policies = resolveToolPolicies({ autoApprove });
		...
		for (const name of [
			...
			expect(policies[name]).toEqual({ autoApprove: false });
		}
		expect(policies.fetch_web_content).toEqual({ autoApprove: true });
		expect(policies.ask_question).toEqual({ autoApprove: true });
		expect(resolveToolPolicies({ autoApproveTools: false })["*"]).toEqual({
			autoApprove: false,
		});
	});
	it("adds the preferred language without replacing Core's Plan prompt", async () => {
		const prompt = await resolveSystemPrompt({
			cwd: "/tmp",
			mode: "plan",
			preferredLanguage: "zh-CN",
		});
		expect(prompt).toContain("Speak in zh-CN.");
		expect(prompt).toContain("plan");
		expect(prompt).not.toContain("switch_to_act_mode tool");
		expect(
...
describe("pathless session starts", () => {
	it.each([
		"sessionId",
		"session_id",
	])("names a new chat with a webview-preallocated %s after start, attach, send and completion", async (idField) => {
		const sessionId = `title-lifecycle-${idField}`;
		let persisted = false;
		const get = vi.fn(async () =>
			persisted
				? { sessionId, status: "idle", metadata: { title: "你是谁？" } }
				: null,
		);
		const start = vi.fn(async () => {
			persisted = true;
			return { sessionId, manifest: { cwd: "/tmp", workspace_root: "/tmp" } };
		});
		...
		const generate = vi
			.spyOn(sessionTitle, "generateSessionTitle")
			.mockResolvedValue(undefined);
		const ctx = {
			liveSessions: new Map(),
			restoringWorkspacePaths: new Set(),
			wsClients: new Set(),
			sessionManager: { get, start, send },
		...
		try {
			...
			expect(live.autoTitleEligible).toBe(true);
			await handleChatSessionCommand(ctx, { action: "attach", sessionId });
			expect(ctx.liveSessions.get(sessionId)).toBe(live);
			...
			expect(live.autoTitlePrompt).toBe("你是谁？");
			handleCoreSessionEvent(ctx, {
				type: "agent_event",
				payload: {
					sessionId,
					event: { type: "done", reason: "completed", text: "我是 AI 助手。" },
				},
			} as Parameters<typeof handleCoreSessionEvent>[1]);
			expect(generate).toHaveBeenCalledWith(ctx, sessionId, "我是 AI 助手。");
		} finally {
	...
	it.each([
		"persisted",
		"imported",
	])("does not automatically rename %s history", async (kind) => {
		const sessionId = `title-existing-${kind}`;
		const ctx = {
			liveSessions: new Map(),
			restoringWorkspacePaths: new Set(),
			sessionManager: {
				get: vi.fn(async () =>
					kind === "persisted" ? { sessionId, status: "idle" } : null,
				),
				start: vi.fn(async () => ({
					sessionId,
					manifest: { cwd: "/tmp", workspace_root: "/tmp" },
				})),
			},
		} as unknown as SidecarContext;
		await handleChatSessionCommand(ctx, {
			...
			config: {
				...
				systemPrompt: "test",
				...(kind === "imported"
					? { initialMessages: [{ role: "user", content: "历史提问" }] }
					: {}),
			},
		});
		expect(ctx.liveSessions.get(sessionId)?.autoTitleEligible).toBe(false);
	});
	it("omits workspace paths and returns the SDK-resolved chat workspace", async () => {
		...
		expect(ctx.liveSessions.get("session-pathless")?.autoTitleEligible).toBe(
			true,
		);
	});
});
...
describe("first-send connection updates", () => {
	...
	it("captures only the first prompt for automatic naming", async () => {
		const { ctx, sessionId } = createContext();
		const session = ctx.liveSessions.get(sessionId)!;
		session.autoTitleEligible = true;
		session.compactionConfigured = true;
		await handleChatSessionCommand(ctx, {
			action: "send",
			sessionId,
			prompt: "首问".repeat(1200),
		});
		expect(session.autoTitlePrompt).toHaveLength(1800);
		await handleChatSessionCommand(ctx, {
			action: "send",
			sessionId,
			prompt: "后续提问",
		});
		expect(session.autoTitlePrompt).toBe("首问".repeat(900));
	});

	it.each([
		true,
		false,
	])("migrates an old idle runtime once before sending (config supplied: %s)", async (withConfig) => {
		const { ctx, sessionId, start, stop, send } = createContext({
			attachedViaHub: true,
		});
		const live = ctx.liveSessions.get(sessionId)!;
		live.compactionConfigured = false;
		await handleChatSessionCommand(ctx, {
			action: "send",
			sessionId,
			prompt: "continue",
			...(withConfig ? { config: { ...baseConfig } } : {}),
		});
		expect(stop).toHaveBeenCalledOnce();
		expect(start).toHaveBeenCalledWith(
			expect.objectContaining({
				config: expect.objectContaining({
					compaction: resolveDesktopCompactionConfig(),
				}),
				initialMessages: [
					{ role: "user", content: "first prompt" },
					{ role: "assistant", content: "first response" },
		...
		expect(live.compactionConfigured).toBe(true);
		await handleChatSessionCommand(ctx, {
			action: "send",
			sessionId,
	...
	it("defers old-runtime migration while a turn is running or prompts are queued", async () => {
		...
		expect(live.compactionConfigured).toBe(false);
		live.busy = false;
		live.status = "idle";
		await handleChatSessionCommand(ctx, {
			action: "send",
		...
		await handleChatSessionCommand(ctx, {
			action: "send",
			sessionId,
			prompt: "next idle turn",
		});
		expect(start).toHaveBeenCalledOnce();
		expect(live.compactionConfigured).toBe(true);
	});

	it("does not mark a failed migration applied or submit the prompt", async () => {
		const { ctx, sessionId, start, send } = createContext();
		const live = ctx.liveSessions.get(sessionId)!;
		live.compactionConfigured = false;
		...
		await expect(
			...
			result: { finishReason: "error", text: "start failed" },
		});
		expect(send).not.toHaveBeenCalled();
		expect(live.compactionConfigured).toBe(false);
	...
	it("keeps a locally running session on one event channel after repeated re-attach", async () => {
		const { ctx, sessionId } = createContext();
		const live = ctx.liveSessions.get(sessionId)!;
		live.busy = true;
		live.status = "running";
		...
		const send = vi.fn();
		ctx.wsClients.add({ send } as never);
		...
		expect(live.attachedViaHub).toBe(false);
		expect(command).not.toHaveBeenCalled();
		handleCoreSessionEvent(ctx, {
			type: "agent_event",
			payload: {
				sessionId,
				event: { type: "content_start", contentType: "text", text: "你好" },
			},
		} as Parameters<typeof handleCoreSessionEvent>[1]);
		...
		const chunks = send.mock.calls
			...
			.filter((event) => event.event?.payload?.stream === "chat_text");
		expect(chunks).toHaveLength(1);
		expect(chunks[0].event.payload.chunk).toBe("你好");
	});

	it("attaches unknown sessions through the Hub", async () => {
		const { ctx, sessionId } = createContext();
		...
		expect(attached).not.toHaveProperty("autoApprove");
		expect(command).toHaveBeenCalledWith(
			"session.attach",
			{ sessionId },
			sessionId,
		);
		expect(ctx.liveSessions.get(sessionId)?.attachedViaHub).toBe(true);
		expect(ctx.liveSessions.get(sessionId)?.compactionConfigured).toBe(false);
	});
	...
	it.each([
		...
		[{ mode: "act" }, { mode: "plan" }],
		[{ mode: "plan" }, { mode: "act" }],
		[{ preferredLanguage: "zh-CN" }, { preferredLanguage: "en" }],
		[{ preferredLanguage: "en" }, { preferredLanguage: "default" }],
	])("rebuilds Core for mode/language changes, preserving history: %j to %j", async (previous, next) => {
		const { ctx, sessionId, start, stop, send } = createContext({
		...
		expect(start).toHaveBeenCalledWith(
			expect.objectContaining({
				config: expect.objectContaining({
					mode: "mode" in next ? next.mode : "act",
					systemPrompt:
						"preferredLanguage" in next && next.preferredLanguage !== "default"
							? expect.stringContaining(`Speak in ${next.preferredLanguage}.`)
							: expect.not.stringContaining("# Preferred Language"),
				}),
				initialMessages: [
					{ role: "user", content: "first prompt" },
					{ role: "assistant", content: "first response" },

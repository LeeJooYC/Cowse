...
describe("runCancellableProviderOAuthLogin", () => {
	...
	it("does not report success when saving browser credentials fails", async () => {
		const save = vi.fn(() => {
			throw new Error("credential persistence failed");
		});
		const { dependencies } = makeDependencies({
			login: async () => ({ accessToken: "test-only-token" }),
			save,
		});
		await expect(
			runCancellableProviderOAuthLogin(
		...
		expect(cancelProviderOAuthLogin("openai-codex")).toBe(false);
	});

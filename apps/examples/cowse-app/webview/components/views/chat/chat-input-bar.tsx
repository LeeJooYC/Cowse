...
import { resolveAutoApprove, type AutoApprove } from "@/lib/auto-approve";
...
function resolveEffortIndex(
	...
	return thinking === null ? 0 : index >= 0 ? index : 0;
}
...
type ChatInputBarProps = {
	...
	onModelSettingsChange?: (settings: ModelRuntimeSettings) => void;
	mode: "act" | "plan";
	thinking: ChatSessionConfig["thinking"];
	reasoningEffort: ChatSessionConfig["reasoningEffort"];
...
function ChatInputBarImpl({
	...
	const [reasoningCapability, setReasoningCapability] = useState<{
		...
		options?: ProviderModel["reasoningOptions"];
	} | null>(null);
	const handleModelSupportsReasoningChange = useCallback(
		(
			...
			setReasoningCapability((current) => {
				...
				return { provider, model, supported, options };
			});
		},
		[model, provider],
	);
	...
	const activeSettings =
		modelSettings?.provider === provider && modelSettings.model === model
			? modelSettings
			: undefined;
	const availableEfforts = reasoningChoices(
	...
	const handleEffortChange = useCallback(
		(value: string) => {
			if (isBusy || !availableEfforts.includes(value)) {
				return;
			}
		...
		[availableEfforts, isBusy, onReasoningChange],
	);
	...
	const renderTokenUsage = (editor?: React.ReactNode) => (
		<TokenUsageRing
			editor={editor}
			usage={{
				contextWindow: activeSettings?.contextWindow ?? modelContextWindow,
				...
				<div className="relative">
					...
					>
						...
						<textarea
							...
							onKeyDown={(e) => {
								// Enter confirms a CJK input-method candidate while composition
								// is active; it must never be treated as message submission.
							...
							placeholder={
								speechInputProcessing
									? "正在转写语音…"
									: variant === "welcome"
										? "给牛马发消息，@ 文件、引用 #PR 或使用 / 命令"
										: isBusy
											? "任务正在运行，可继续输入并排队发送"
											: "给牛马发消息，输入 / 查看命令，输入 @ 添加上下文"
							}
			...
			<div className="flex min-w-0 flex-wrap items-center justify-between gap-x-3 gap-y-2 rounded-b-xl border-t border-border bg-muted/20 px-2 py-2 text-sm text-muted-foreground">
				...
				>
					...
					>
						<SelectTrigger
							aria-label="思考强度"
							className="size-7 justify-center border-0 p-0 shadow-none data-[size=sm]:h-7 [&>svg:last-child]:hidden bg-transparent! hover:bg-surface-hover!"
							size="sm"
							title={`思考强度：${effortLabel}`}
						>
						...
						<SelectContent align="end">
							{EFFORT_LEVELS.filter((option) =>
								availableEfforts.includes(option.value),
							).map((option) => (
...
const ModelSelector = memo(function ModelSelector({
	...
	useEffect(() => {
		let cancelled = false;
		const revision = ++modelsRevisionRef.current;
		...
		async function loadCatalogAndActiveModels() {
			...
			if (
				!normalizedProvider ||
				cancelled ||
				revision !== modelsRevisionRef.current
			) {
				return;
			}
			try {
	...
	const refreshActiveModels = useCallback(async () => {
		if (!resolvedProvider || isBusy) return;
		const revision = ++modelsRevisionRef.current;
		setRefreshingProvider(resolvedProvider);
		try {
			const models = await loadProviderModels(resolvedProvider, {
				fresh: true,
			});
			if (revision !== modelsRevisionRef.current) return;
			setProviderModels((current) => ({
				...current,
				[resolvedProvider]: models.map((entry) => entry.id),
			}));
			setProviderReasoningModels((current) => ({
				...current,
			...
			setReasoningCapabilitySource("catalog");
			setRefreshingProvider(null);
			publishProviderModels(resolvedProvider, models);
		} catch (error) {
			if (revision !== modelsRevisionRef.current) return;
			toast({
				title: "刷新模型失败",
				description: `暂时保留上次列表：${String(error)}`,
				variant: "destructive",
			});
		} finally {
			if (revision === modelsRevisionRef.current) setRefreshingProvider(null);
		}
	}, [resolvedProvider, isBusy]);
	...
	useEffect(() => {
		...
		onModelSupportsReasoningChange(
			...
			modelDetails[normalizedProvider]?.find((entry) => entry.id === model)
				?.reasoningOptions,
		);
	}, [
		model,
	...
	const renderModelSelect = (
		...
		<SearchCombobox
			ariaLabel="Model"
			className={triggerClassName}
			disabled={isBusy || !resolvedProvider}
			emptyText="未找到可用模型。"
			loading={refreshingProvider === resolvedProvider}
			loadingText="正在刷新模型…"
			onOpen={() => {
...
function TokenUsageRing({
	...
	const knownWindow =
		usage.contextWindow && usage.contextWindow > 0
			? usage.contextWindow
			: undefined;
	const contextWindow = knownWindow ?? 1;
	const totalTokens = usage.tokensIn + usage.tokensOut;

	const ratio = Math.min(
		knownWindow ? Math.max(totalTokens / Math.max(contextWindow, 1), 0) : 0,
		1,
	);
	...
	const contextUsageLabel = knownWindow
		? `${formatCompactTokens(totalTokens)} / ${contextWindow % 1024 === 0 ? formatContextBudget(contextWindow) : formatCompactTokens(contextWindow)} (${percent}%)`
		: `${formatCompactTokens(totalTokens)} / 未知`;
	...
	const segmentWidth = (tokens: number) =>
		`${knownWindow ? (tokens / contextWindow) * segmentScale * 100 : 0}%`;
	const radius = 8.5;
		...
		<Popover>
			<PopoverTrigger asChild>
				<Button
					aria-label={
						knownWindow
							? `上下文窗口：已使用 ${totalTokens.toLocaleString()} / ${contextWindow.toLocaleString()} Token（${percent}%）`
							: `上下文窗口：已使用 ${totalTokens.toLocaleString()} Token，上限未知`
					}
			...
			<PopoverContent
				align="end"
				className="w-80 max-h-[75vh] overflow-y-auto p-0"
			...
			>
				<div className="px-3 py-3">
					<div className="flex items-center justify-between gap-4 text-sm">
						<span className="text-muted-foreground">上下文预算</span>

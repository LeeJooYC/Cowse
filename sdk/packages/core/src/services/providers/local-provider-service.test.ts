...
describe("refreshProviderModelsFromSource", () => {
	...
	it("refreshes Moonshot from the saved compatible endpoint without bundled models", async () => {
		const fetchMock = vi.fn().mockResolvedValue({ok: true, json: async () => ({data: [{id: "coding-only", context_length: 1048576, max_output_tokens: 32768, supported_reasoning_efforts: ["low", "high"]}]})});
		vi.stubGlobal("fetch", fetchMock);
		...
		expect(result.models[0]?.reasoningOptions).toEqual([{type: "effort", values: ["low", "high"]}]);
		...
		expect(refreshed.models[0]?.reasoningOptions).toBeUndefined();
		const documented = LlmsModels.getGeneratedModelsForProvider("moonshotai-cn")["kimi-k3"]?.reasoningOptions;
		expect(documented?.length).toBeGreaterThan(0);
		fetchMock.mockResolvedValue({ok: true, json: async () => ({data: [{id: "kimi-k3"}]})});
		await refreshProviderModelsFromSource(manager, "moonshotai-cn");
		const withFallback = await getLocalProviderModels("moonshotai-cn", manager.getProviderConfig("moonshotai-cn", {includeKnownModels: false}));
		expect(withFallback.models[0]?.reasoningOptions).toEqual(documented);
		...
		const list = async () =>
			(
				await getLocalProviderModels(
					"openai-compatible",
					manager.getProviderConfig("openai-compatible", {
						includeKnownModels: false,
					}),
				)
			).models.map((model) => model.id);
		await refreshProviderModelsFromSource(manager, "openai-compatible");
		expect(await list()).toEqual(["old-model"]);

		ids = ["new-chat", "new-reasoner"];
		await refreshProviderModelsFromSource(manager, "openai-compatible");
		expect(await list()).toEqual(ids);
		expect(await list()).toEqual(ids);
		const stored = await readModelsFile(resolveModelsRegistryPath(manager));
		expect(
			Object.keys(stored.providers["openai-compatible"].models ?? {}),
		).toEqual(ids);
	});

	it("preserves the saved list when refreshing the endpoint fails", async () => {
		...
		await refreshProviderModelsFromSource(manager, "openai-compatible");
		const before = await readModelsFile(resolveModelsRegistryPath(manager));
		fetchMock.mockRejectedValueOnce(new Error("connection refused"));
		await expect(
			refreshProviderModelsFromSource(manager, "openai-compatible"),
	...
	it("refreshes OpenAI-compatible models from the configured /models endpoint without keeping stale defaults", async () => {
		const fetchMock = vi.fn().mockResolvedValue({
			ok: true,
			json: async () => ({ data: [{ id: "live-local-model" }] }),
		});
		vi.stubGlobal("fetch", fetchMock);
		saveLocalProviderSettings(manager, {
			providerId: "openai-compatible",
		...
		const result = await refreshProviderModelsFromSource(
			manager,
			"openai-compatible",
		);
		...
		expect(models.map((model) => model.id)).toEqual(["live-local-model"]);
	});
});

...
function tryGetModelInfo(config: AgentConfig): ModelInfo | undefined {
	// Use the same resolved definition as the inference gateway. Reading the
	// raw catalog first bypasses per-session modelInfo/maxInputTokens overrides,
	// so compaction can silently use a 1M window while the host budget is 256K.

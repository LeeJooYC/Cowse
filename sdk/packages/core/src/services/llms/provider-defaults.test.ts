...
describe("resolveProviderConfig", () => {
	it("does not let an invalidated in-flight model request restore the old cache", async () => {
		...
		const list = () =>
			resolveProviderConfig("openai-compatible", undefined, config);
		const oldRequest = list();
		await vi.waitFor(() => expect(fetchMock).toHaveBeenCalledTimes(1));
		clearPublicModelsCatalogCache(config);
		const newRequest = list();
		await vi.waitFor(() => expect(fetchMock).toHaveBeenCalledTimes(2));
		finishOld(Response.json({ data: [{ id: "old-model" }] }));
		await oldRequest;
		const concurrentReader = list();
		finishNew(Response.json({ data: [{ id: "new-model" }] }));
		for (const result of await Promise.all([
			newRequest,
			concurrentReader,
			list(),
		])) {
			expect(Object.keys(result?.knownModels ?? {})).toEqual(["new-model"]);
		}
		expect(fetchMock).toHaveBeenCalledTimes(2);
	});

	it("invalidates only the refreshed provider endpoint", async () => {
		...
		const second = { ...first, baseUrl: "http://localhost:8081/v1" };
		await resolveProviderConfig(first.providerId, undefined, first);
		await resolveProviderConfig(second.providerId, undefined, second);
		clearPublicModelsCatalogCache(first);
		await resolveProviderConfig(second.providerId, undefined, second);
		expect(fetchMock).toHaveBeenCalledTimes(2);
		await resolveProviderConfig(first.providerId, undefined, first);

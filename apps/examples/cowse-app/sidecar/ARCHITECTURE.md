...
The composer stores five opt-in categories in `config.autoApprove`. The sidecar projects them into Core tool policies: read/search, editor/patch, commands, web fetch, and `mcp:*`. Unknown tools require approval; ask-question and submit/finish tools do not need a redundant approval. Legacy configs retain their prior global policy. Changes apply on the next send through the existing history-preserving runtime rebuild; editing is disabled while running. Core remains responsible for executing tools and requesting approval.

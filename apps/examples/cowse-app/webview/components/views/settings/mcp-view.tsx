...
export function McpServersContent({
	...
	const renderServerDetails = (server: McpServer) => (
		<div className="flex flex-col gap-1 text-xs text-muted-foreground">
			{server.command && (
				<p>
					<span className="text-muted-foreground/70">启动命令：</span>{" "}
					{server.command}
				</p>
			)}
			...
			{server.url && (
				<p>
					<span className="text-muted-foreground/70">地址：</span> {server.url}
				</p>
			)}
				...
				<div className="mt-2.5 grid gap-2">
					{server.configurationError ? (
						<div
							...
							role="alert"
						>
							<p className="text-xs font-medium text-destructive">配置无效</p>
						...
						>
							<div className="min-w-0 flex-1 grid gap-0.5">
								<p className="text-xs font-medium text-foreground">
									{isAuthorizing ? "正在等待 OAuth 授权" : "需要 OAuth 授权"}
								</p>
								<p className="wrap-break-word text-xs text-muted-foreground">
									{isAuthorizing
										? "请在浏览器中完成登录，或点击“取消”停止等待。"
										: (serverError ??
											"点击“连接”在浏览器中登录。授权成功前，服务将保持停用。")}
								</p>
							</div>
							<Button
								variant="default"
					...
					{serverError && !server.oauthStatus?.authorizationRequired ? (
						<div
							...
							role="alert"
						>
							<p className="text-xs font-medium text-destructive">连接失败</p>
							<p className="mt-0.5 wrap-break-word text-xs text-muted-foreground">
								{serverError}
							</p>
						</div>
		...
		<>
			...
			>
				<DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
					<DialogHeader>
						<DialogTitle>
							{editorMode === "edit" ? "编辑 MCP 服务" : "添加 MCP 服务"}
						</DialogTitle>
						<DialogDescription>
							{editorMode === "edit"
								? "MCP 服务配置将更新至："
					...
					<div className="grid gap-4">
						...
						{formState.transportType === "stdio" ? (
							<>
								...
								<div className="grid gap-2">
									<Label htmlFor="mcp-args">参数（用逗号分隔）</Label>
									<Textarea
										id="mcp-args"
							...
							<>
								...
								<div className="grid gap-2">
									<Label htmlFor="mcp-headers">请求头（用逗号分隔）</Label>
									<Textarea
										id="mcp-headers"
						...
						<div className="flex items-center justify-between rounded-md border border-border px-3 py-2">
							<div>
								<p className="text-sm font-medium text-foreground">启用服务</p>
			...
			>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							{deleteTarget
								? `确定从配置中删除 MCP 服务“${deleteTarget.name}”吗？`
								: "确定从配置中删除此 MCP 服务吗？"}
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>

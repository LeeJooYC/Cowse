...
```
# Composer auto-approval

The composer stores five opt-in categories in `config.autoApprove`: read/search, editor/patch, commands, web fetch, and `mcp:*`. Category tools use Core's approval callback, where the sidecar reads the live session config instead of a frozen start-time policy. Unknown tools require approval; ask-question and submit/finish tools do not need a redundant approval. Legacy configs retain their prior global policy. Changes apply on the next send or when responding to a pending approval. The response carries the current choices; the sidecar validates the trusted connection, request ownership, session and schema before saving choices and resuming the tool. Already pending approvals are not silently accepted. Editing is enabled while waiting for approval, but disabled during other active execution. Core remains responsible for executing tools and requesting approval.

...
export type DesktopThread = {
	id: string;
	/** Cline session backing a newly-created live thread. */
				...
				state.threads
					.filter(
						(thread) =>
							...
							thread.activeSessionId === action.deletedSessionId ||
				...
				threads: state.threads.map((thread) =>
					...
					(!thread.hasStarted ||
						(action.sessionId !== undefined &&
							thread.activeSessionId !== action.sessionId))
						? {
								...thread,
								hasStarted: true,
								activeSessionId: action.sessionId ?? thread.activeSessionId,
							}
						: thread,
				),
			};

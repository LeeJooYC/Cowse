...
async function exchangeAuthorizationCode(
	...
	if (!response.ok) {
		// Do not discard the response status: browser callback success happens
		// before this request, so hosts need to distinguish a rejected exchange
		// from a UI synchronization failure. Never echo the raw response body or
		// description, which may contain credentials or callback parameters.
		const details = parseOAuthError(await response.text().catch(() => ""));
		...
		const errorCode =
			details.code && safeCodes.has(details.code) ? details.code : undefined;
		throw new OpenAICodexOAuthTokenError(
			`Token exchange failed: HTTP ${response.status}${errorCode ? ` (${errorCode})` : ""}`,
	...
	if (
		...
		throw new OpenAICodexOAuthTokenError(
			"Token exchange failed: invalid token response (missing or invalid required fields)",

...
async function renderControls({
	...
	onRemoveWorkspace = vi.fn(async () => true),
}: {
...
describe("WelcomeWorkspaceControls manual path entry", () => {
	it("removes a recent folder without invoking filesystem deletion", async () => {
		const onRemoveWorkspace = vi.fn(async () => true);
		await renderControls({ onRemoveWorkspace });
		await openWorkspaceMenu();

		const removeButton = container.querySelector<HTMLButtonElement>(
			'button[aria-label="从最近使用中移除 /projects/project-1"]',
		);
		...
		expect(onRemoveWorkspace).toHaveBeenCalledWith("/projects/project-1");
	});

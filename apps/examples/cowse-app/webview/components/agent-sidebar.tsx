...
const SETTINGS_SECTION_ICONS = {
	...
	AccountUsage: CircleUserRound,
	AccountBilling: CircleUserRound,
	Customize: Blocks,
	Marketplace: Store,
...
function settingsSectionLabel(section: SettingsSection): string {
	const labels: Partial<Record<SettingsSection, string>> = {
		General: "通用",
		Models: "模型",
		Voice: "语音",
		Channels: "频道",
...
function SettingsSectionNavigation({
	...
	const accountOpen = (ACCOUNT_SECTIONS as readonly SettingsSection[]).includes(
		activeSection,
	);
	const extensionsOpen = (
	...
	const renderChild = (
		...
		<Button
			...
			className={cn(
				"min-w-0 pl-8!",
				activeSection === section && "bg-surface-hover text-sidebar-foreground",
			)}
			disabled={disabled}
	...
	return (
		...
		>
			...
			{SETTINGS_SECTIONS.filter(
				(section) => section !== "Schedules" && section !== "Account",
			...
			<Button
				...
				className={cn(
					"min-w-0 justify-start",
					collapsed && "size-9 justify-center px-0",
					extensionsOpen && "bg-surface-hover-lighter text-sidebar-foreground",
				)}
			...
			>
				...
				{!collapsed && <span className="truncate">扩展</span>}
			</Button>
			{extensionsOpen &&
				!collapsed &&
				CUSTOMIZATION_SECTIONS.map((section) =>
					renderChild(section, settingsSectionLabel(section)),
				)}
			<Button
				aria-label="账户"
				className={cn(
					"min-w-0 justify-start",
					collapsed && "size-9 justify-center px-0",
					accountOpen && "bg-surface-hover-lighter text-sidebar-foreground",
				)}
			...
			{accountOpen &&
				...
				ACCOUNT_SECTIONS.map((section) =>
					renderChild(
						section,
						ACCOUNT_SECTION_LABELS[section],
...
export function AgentSidebar({
	...
	const schedulesActive =
		view === "settings" && settingsSection === "Schedules";
	const settingsNavigationActive = view === "settings" && !schedulesActive;
	const {
	...
	const loadProcessContext = useCallback(async () => {
		try {
			...
			setHubStatus({
				connected: false,
				error:
					error instanceof Error ? error.message : "无法读取 Cline Core 状态。",
				url: null,
			});
		}
	}, []);
			...
			<div className="flex h-full min-h-0 w-full min-w-0 shrink-0 flex-col overflow-hidden bg-sidebar text-sidebar-foreground">
									...
									<div className="mt-3 border-border border-t pt-3">
										<div className="flex items-center gap-2 text-xs">
											...
											<span className="font-medium">
												Cline Core @{hubPort(hubStatus?.url ?? null) ?? "未知"}
											</span>
										</div>
										{hubStatus && !hubStatus.connected && (
											<p className="mt-1 text-[11px] text-destructive">
												{hubStatus.error ?? "Cline Core 尚未连接。"}
											</p>
										)}
									</div>
								</HoverCardContent>
				...
				{settingsNavigationActive && (
					<Button
						aria-label="返回应用"
						className={cn(
							"mx-2 mt-2 w-auto justify-start",
							isCollapsed && "mx-1.5 size-9 justify-center px-0",
						)}
						onClick={() => {
							setView("chat");
							closeMobileSidebar();
					...
					>
						...
						{!isCollapsed && <span>返回应用</span>}
					</Button>
				)}

				{!isCollapsed && !settingsNavigationActive ? (
					<nav
						aria-label="侧边栏操作"
					...
					>
						...
						<Button
							...
							aria-label="计划任务"
							className={cn(
								schedulesActive && "bg-surface-hover text-sidebar-foreground",
							)}
						...
						>
							...
							<span className="truncate">计划任务</span>
						</Button>
					</nav>
				) : null}
				...
				>
					{user && !isCollapsed ? (
						<div className="flex min-w-0 items-center gap-2">
							<div
								aria-label="用户信息"
								className="flex min-w-0 flex-1 items-center gap-2.5 rounded-md p-2 text-left text-sidebar-foreground"
			...
			>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							这会从 Cline Core 的本地记录中删除“
							{normalizeTitle(deleteConfirmThread?.title ?? "此会话")}”。
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
...
export function getSessionOverviewItems(
	...
	const items: Array<[string, string | null | undefined, string?]> = [
		...
		["供应商", thread.provider],
		["模型", thread.model],
		["Token 用量", formatTokenCount(thread.inputTokens, thread.outputTokens)],
		["费用", formatCostUsd(thread.totalCostUsd)],
		["来源", thread.source ? getSessionSourceLabel(thread.source) : null],
	];
	...
	return (
		<ContextMenuContent className="w-40">
			...
			<ContextMenuItem disabled={pending} onSelect={onRename}>
				...
				{pendingAction === "rename" ? "正在重命名…" : "重命名"}
			</ContextMenuItem>
			<ContextMenuItem disabled={pending} onSelect={onFork}>
				...
				{pendingAction === "fork" ? "正在创建分支…" : "创建会话分支"}
			</ContextMenuItem>
			<ContextMenuItem
				disabled={pending}
				onSelect={onDelete}

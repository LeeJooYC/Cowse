...
beforeEach(() => {
	...
	loadProviderModelsMock
		...
		.mockImplementation(async (provider: string) => {
			const catalog = await loadProviderModelCatalogMock.mock.results[0]?.value;
			return (catalog?.providerModels?.[provider] ?? []).map(
				(id: string) =>
					catalog?.providerModelDetails?.[provider]?.find(
...
describe("ChatInputBar", () => {
	it("refreshes models on opening without Settings and rejects late stale loads", async () => {
		...
		loadProviderModelsMock
			...
			.mockResolvedValueOnce([{ id: "new-backend", name: "New backend" }]);
		await renderVoiceComposer();
		expect(loadProviderModelsMock).toHaveBeenCalledWith("cline", {
			fresh: true,
		});
		const trigger = () =>
			container.querySelector<HTMLButtonElement>('[aria-label^="Model:"]');
		...
		await act(async () =>
			stale.resolve([{ id: "old-backend", name: "Old backend" }]),
		);
		expect(
			container.querySelector('[role="dialog"]')?.textContent,
		).not.toContain("Old backend");
		await act(async () => trigger()?.click());
		loadProviderModelsMock.mockResolvedValueOnce([
			{ id: "third-backend", name: "Third backend" },
		]);
		await act(async () => trigger()?.click());
		expect(container.querySelector('[role="dialog"]')?.textContent).toContain(
			"Third backend",
		...
		expect(
			container.querySelector('[role="dialog"]')?.textContent,
		).not.toContain("New backend");
		await act(async () => trigger()?.click());
		loadProviderModelsMock.mockRejectedValueOnce(new Error("offline"));
		await act(async () => trigger()?.click());
		expect(container.querySelector('[role="dialog"]')?.textContent).toContain(
			"Third backend",
		);
	...
	it("groups localized mode, icon-only thinking and context on the right", async () => {
		const onModeToggle = vi.fn();
		await renderVoiceComposer({ mode: "act", onModeToggle });
		const group = container.querySelector('[aria-label="会话模式"]');
		expect(group?.classList.contains("hidden")).toBe(false);
		const controls = container.querySelector('[aria-label="会话控制"]');
		expect(controls?.classList.contains("ml-auto")).toBe(true);
		expect(controls?.firstElementChild?.getAttribute("aria-label")).toBe(
			"会话模式",
		);
		expect(group?.nextElementSibling?.getAttribute("aria-label")).toBe(
			"自动允许",
		);
		expect(group?.textContent).toBe("规划执行");
		expect(
			group?.nextElementSibling?.nextElementSibling?.getAttribute("aria-label"),
		).toBe("思考强度");
		const thinking = controls?.querySelector('[aria-label="思考强度"]');
		expect(thinking?.querySelector(".sr-only")).not.toBeNull();
		expect(thinking?.getAttribute("title")).toContain("思考强度：");
		expect(controls?.querySelectorAll("button").item(4)?.id).toBe(
			"token-usage",
		);
		...
		expect(
			container
				.querySelector('[aria-label="规划模式"]')
				?.getAttribute("aria-pressed"),
		).toBe("true");
		const actButton = container.querySelector<HTMLButtonElement>(
			'[aria-label="执行模式"]',
		);
	...
	it("opens five approval categories and disables changes while running", async () => {
		const onAutoApproveChange = vi.fn();
		await renderVoiceComposer({ onAutoApproveChange });
		await act(async () =>
			container
				.querySelector<HTMLButtonElement>('[aria-label="自动允许"]')
				?.click(),
		);
		...
		expect(onAutoApproveChange).toHaveBeenCalledWith({
			read: true,
			edit: true,
			commands: false,
			web: true,
		...
		await renderVoiceComposer({ status: "running", onAutoApproveChange });
		expect(
	...
	it("protects the draft and send action for the full streaming transcription lifecycle", async () => {
		...
		expect(textarea?.placeholder).toBe("正在转写语音…");
	...
	it("normalizes an unavailable strength to explicit on across status updates", async () => {
		...
		expect(leftControls?.contains(workspaceTrigger ?? null)).toBe(true);
		expect(
			container
				.querySelector('[aria-label="会话控制"]')
				?.contains(thinkingTrigger ?? null),
		).toBe(true);
	...
	it("selects Core's High level even when legacy Cowse settings declare different levels", async () => {
	...
	it("does not overwrite the remembered model when rendering a session's provider/model", async () => {
		...
		expect(panel?.querySelector("input")?.placeholder).toBe("搜索模型");
		const option = [
	...
	describe("cline-pass picker offer", () => {
		...
		it("falls back to a visible model when switching providers with a hidden remembered model", async () => {
			...
			const panel = document.querySelector('[role="dialog"]');
			expect(panel?.querySelector("input")?.placeholder).toBe("搜索供应商");
...
describe("ChatInputBar token ring", () => {
	...
	it("initializes a local budget without opening the popover and updates the closed ring on completion", async () => {
		const invoke = vi.spyOn(desktopClient, "invoke").mockResolvedValue({});
		const onChange = vi.fn();
		await renderTokenUsage(
			{ toolCalls: 0, tokensIn: 0, tokensOut: 0 },
			undefined,
			{
		...
		expect(document.querySelector("#token-usage-panel")).toBeNull();
		...
		const modelSettings = onChange.mock.calls.at(-1)![0];
		const running = await renderTokenUsage(
			{ toolCalls: 0, tokensIn: 0, tokensOut: 0 },
			undefined,
			{
				provider: "openai-compatible",
		...
		const completed = await renderTokenUsage(
			{ toolCalls: 0, tokensIn: 32768, tokensOut: 32768 },
			undefined,
			{
				provider: "openai-compatible",
				modelSettings,
		...
		expect(completed?.getAttribute("aria-label")).toContain("50%");
		expect(
			completed
				?.querySelectorAll("circle")[1]
				?.getAttribute("stroke-dashoffset"),
	...
	it("keeps context settings available when the model limit is unknown", async () => {
		expect(
			await renderTokenUsage({
				toolCalls: 0,
				tokensIn: 500,
		...
		expect(outputOnly?.getAttribute("aria-label")).toBe(
			"上下文窗口：已使用 500 / 2,000 Token（25%）",
		);
	});

	it("fills from input and output and sits after thinking controls, separate from workspace", async () => {
		...
		expect(trigger?.getAttribute("aria-label")).toBe(
			"上下文窗口：已使用 1,500 / 2,000 Token（75%）",
		);
		...
		const usageGroup = trigger.closest('[aria-label="会话控制"]');
		const workspaceSelector = container.querySelector("#git-branch-btn");
		if (!workspaceSelector) {
			throw new Error("Expected workspace selector in composer");
		}
		expect(usageGroup?.contains(workspaceSelector)).toBe(false);
		expect(usageGroup?.querySelector('[aria-label="思考强度"]')).not.toBeNull();
		expect(
			Boolean(

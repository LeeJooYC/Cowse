...
describe("formatThoughtLabel", () => {
	it("falls back to a plain label without a duration", () => {
		expect(formatThoughtLabel(undefined)).toBe("正在思考");
	});

	it("rounds to seconds with a one-second floor for nonzero durations", () => {
		expect(formatThoughtLabel(0)).toBe("已思考 0 秒");
		expect(formatThoughtLabel(120)).toBe("已思考 1 秒");
		expect(formatThoughtLabel(2_600)).toBe("已思考 3 秒");
	});
});

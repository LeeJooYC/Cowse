...
const compaction = vi.hoisted(() => ({ active: false }));
vi.mock("@/hooks/use-compaction-status", () => ({
	useCompactionStatus: () => compaction.active,
}));
...
describe("ChatMessages tool disclosures", () => {
	it("shows compaction instead of thinking even after output has started", async () => {
		...
		await renderMessages(
			...
			{ status: "running", streamingMessageId: "answer" },
		);
		...
		expect(container.textContent).not.toContain("正在压缩上下文…");
	...
	it("auto-expands submit_and_exit and renders its summary as markdown", async () => {
		...
		const trigger = [...container.querySelectorAll("button")].find((element) =>
			element.textContent?.includes("计划任务已完成"),
		);
		expect(trigger?.getAttribute("aria-expanded")).toBe("true");
	...
	it("labels an errored submit_and_exit as failed", async () => {
		...
		expect(container.textContent).toContain("计划任务失败");
		expect(container.textContent).not.toContain("计划任务已完成");
	});
	...
	it("keeps the scheduled-task report visible when the run collapses", async () => {
		...
		expect(container.textContent).toContain("计划任务已完成");
	...
	it("renders consecutive tool calls as individual rows", async () => {
		...
		expect(container.textContent).toContain("已读取 2 个文件");
	...
	it("normalizes payload-backed configured subagent names", async () => {
		...
		expect(container.textContent?.match(/已启动智能体/g)).toHaveLength(3);
	...
	it("keeps steering notes hidden inside the expanded work block", async () => {
		...
		].find((button) => button.textContent?.includes("已运行"));
		expect(trigger).toBeDefined();
		await act(async () => trigger?.click());
		expect(container.textContent).not.toContain(
			"This run is not complete until you finish.",
	...
	it("renders live ANSI command output and offers proceed while running", async () => {
		...
		const proceedButton = [...container.querySelectorAll("button")].find(
			(button) => button.textContent?.includes("在后台运行并继续"),
		);
...
describe("ChatMessages reasoning disclosure", () => {
	...
	it("shows elapsed thinking time with the border-left disclosure style", async () => {
		...
		const trigger = [...container.querySelectorAll("button")].find((element) =>
			element.textContent?.includes("已思考 7 秒"),
		);
		expect(trigger?.getAttribute("aria-expanded")).toBe("false");
	...
	it("combines consecutive assistant reasoning into one disclosure", async () => {
		...
		expect(trigger?.textContent).toContain("已思考 2 秒");
	...
	it("falls back to Thinking when there is no previous timestamp", async () => {
		...
		expect(container.textContent).toContain("正在思考");
		expect(container.textContent).not.toContain("已思考");
	});
});
...
describe("ChatMessages work collapse", () => {
	...
	it("folds a finished run into a work summary that expands back into rows", async () => {
		...
		expect(trigger?.textContent).toContain("已运行 4 秒，调用了 2 次工具");
		expect(trigger?.getAttribute("aria-expanded")).toBe("false");
	...
	it("shows while starting", async () => {
		...
		expect(
			[...container.querySelectorAll("span")]
				.find((element) => element.textContent === "正在思考…")
